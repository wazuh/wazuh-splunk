...
/*
With this service, we want to compare policies and determine whether the user policy matches the required policy.

User policies are objects with the same structure returned by the endpoint GET /security/users/me/policies
policy = {
    actions_as_keys: {
    ...
    const result = Object.keys(requirements).reduce((final, action) => {
        if (action in permissions) {
            ...
            const result = resources.reduce((final, resource) => {
                ...
                const filtered = describedResources.filter(value => isMatching(resource, value))
                if (filtered.length) {
                    //Filtered will likely have a single value, but if it has more, since later entries have priority, we only read the last one.
                    return final && permissions[action][filtered[filtered.length - 1]] === 'allow'
                } else {
                    return final && rbac_mode === 'white'
                }
            }, true)
            return final && result
        } else {
 ...
 * or
 *  foo:bar
 * Wildcards for the permission as * are allowed and match any value in its position
 * @param {string} requirement Permissions required
 * @param {string} permission Permissions the user has as returned by GET /security/users/me/policies
 * @returns {boolean}
 */
...
const resourceToObject = (resource, isAllowed) => {
    ...
    if (arr.length === 1) {
        ...
        return { [arr[0]]: resourceToObject(arr.slice(1, arr.length).join(':'), isAllowed) }
    }
}
