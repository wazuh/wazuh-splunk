  ...
  function (
    ...
    CredentialService.checkSelectedApiConnection().then((api) => {
      ...
      const search1 = new SearchManager({
        ...
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\"  | stats count by rule.pci_dss{}",
      ...
      const search2 = new SearchManager({
        ...
        "sample_ratio": 1,
        "latest_time": "$when.latest$",
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\" | stats count by rule.groups",
        "earliest_time": "$when.earliest$",
        "cancelOnUnload": true,
        "app": utils.getCurrentApp(),
        "auto_cancel": 90,
      ...
      const search3 = new SearchManager({
        ...
        "sample_ratio": 1,
        "latest_time": "$when.latest$",
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\" | stats count by agent.name",
        "earliest_time": "$when.earliest$",
        "cancelOnUnload": true,
        "app": utils.getCurrentApp(),
        "auto_cancel": 90,
      ...
      const search4 = new SearchManager({
        ...
        "sample_ratio": 1,
        "latest_time": "$when.latest$",
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\" agent.name=*| chart  count(rule.pci_dss{}) by rule.pci_dss{},agent.name",
        "earliest_time": "$when.earliest$",
        "cancelOnUnload": true,
        "app": utils.getCurrentApp(),
        "auto_cancel": 90,
      ...
      const search5 = new SearchManager({
        ...
        "sample_ratio": 1,
        "latest_time": "$when.latest$",
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\" | stats count sparkline by agent.name, rule.pci_dss{}, rule.description | sort count DESC | rename agent.name as \"Agent Name\", rule.pci_dss{} as Requirement, rule.description as \"Rule description\", count as Count",
        "earliest_time": "$when.earliest$",
        "cancelOnUnload": true,
        "app": utils.getCurrentApp(),
        "auto_cancel": 90,
      ...
      const search7 = new SearchManager({
        ...
        "sample_ratio": null,
        "latest_time": "now",
        "search": "index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"*\"| stats count by \"rule.pci_dss{}\" | sort \"rule.pci_dss{}\" ASC | fields - count",
        "earliest_time": "-24h@h",
        "cancelOnUnload": true,
        "app": utils.getCurrentApp(),
        "auto_cancel": 90,
      ...
      element5.on("click", function (e) {
        if (e.field !== undefined) {
          e.preventDefault()
          const url = TokenUtils.replaceTokenNames("{{SPLUNKWEB_URL_PREFIX}}/app/SplunkAppForWazuh/search?q=index="+IndexService.get() || '*'+" sourcetype=wazuh rule.pci_dss{}=\"$pci$\" | stats count sparkline by agent.name, rule.pci_dss{}, rule.description | sort count DESC | rename agent.name as \"Agent Name\", rule.pci_dss{} as Requirement, rule.description as \"Rule description\", count as Count&earliest=$when.earliest$&latest=$when.latest$", _.extend(submittedTokenModel.toJSON(), e.data), TokenUtils.getEscaper('url'), TokenUtils.getFilters(mvc.Components))
