...
require([
  ...
  function (
    ...
    const pollingErrorToast = new Toast('error', 'toast-bottom-right', 'Could not load Agents History status', 1000, 250, 250)


    CredentialService.checkSelectedApiConnection().then(({ api, selectedIndex }) => {
      CredentialService.checkPollingState().then(pollingActive => {
        let element9
        let search9
        ...
        if (!pollingActive) {
          ...
          let filter = (api.filter[0] === 'manager.name') ? 'manager_host' : 'cluster.name'
          filter+='='+api.filter[1]
          search9 = new SearchManager({
            "id": "search9",
            "earliest_time": "$when.earliest$",
