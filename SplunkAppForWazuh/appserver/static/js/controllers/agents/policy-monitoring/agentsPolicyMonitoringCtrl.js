  ...
  class AgentsPM extends DashboardMain {
    ...
    constructor(
      ...
      this.vizz = [
        ...
        new PieChart(
          'ruleDistribution',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} rule.description=* | top rule.description`,
          'ruleDistribution',
          this.scope
        ),
        new PieChart(
          'topPciDss',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} rule.pci_dss{}=* | top  rule.pci_dss{}`,
          'topPciDss',
          this.scope
        ),
        new AreaChart(
          'eventsPerAgent',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} | timechart span=2h count by agent.name  `,
          'eventsPerAgent',
          this.scope,
          { customAxisTitleX: 'Time span' }
        ),
        new Table(
          'alertsSummary',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} |stats count sparkline by agent.name, rule.description, title | sort count DESC | rename rule.description as "Rule description", agent.name as Agent, title as Control`,
          'alertsSummary',
          this.scope
        ),
