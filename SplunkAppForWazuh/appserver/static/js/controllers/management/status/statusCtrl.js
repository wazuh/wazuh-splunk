...
function (controllers) {
  ...
  class Status{
    constructor($scope, $requestService, $notificationService,statusData, agentInfo) {
      this.scope = $scope
      this.scope.load = true
      this.apiReq = $requestService.apiReq
      this.toast = $notificationService.showSimpleToast
      const parsedStatusData = statusData.map(item => item && item.data && item.data.data ? item.data.data : item)
      const [
        summary,
        nodeStatus,
        nodeInfo,
    ...
    $onInit(){
      ...
      if (this.status && this.status.enabled === 'yes' && this.status.running === 'no' ) {
        this.scope.clusterError = 'This cluster is enabled but not running. Please check your cluster health.'
    ...
    */
    async bindStatus () {
      try{
        ...
        this.scope.getDaemonStatusClass = daemonStatus => (daemonStatus === 'running') ? 'status teal' : 'status red'
