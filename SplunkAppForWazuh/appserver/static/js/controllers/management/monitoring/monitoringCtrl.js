  ...
  class Monitoring {
    ...
    constructor(
      ...
      this.vizz = [
        ...
        new LinearChart(
          'alertNodeSummary',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} | timechart span=1h count by cluster.node`,
          'alertNodeSummary',
          this.scope,
          { customAxisTitleX: 'Time span' }
        ),
        new PieChart(
          'topNodes',
          `${this.filters} sourcetype=${this.currentDataService.getSourceType().sourceType} | top cluster.node`,
          'topNodes',
          this.scope
        ),
        new ColumChart(
    ...
    launchSearches() {
      this.vizz[3].changeSearch(
        `${this.filters} cluster.node=${this.scope.currentNode.name} sourcetype=${this.currentDataService.getSourceType().sourceType} | timechart span=2h count`
      )
    }
