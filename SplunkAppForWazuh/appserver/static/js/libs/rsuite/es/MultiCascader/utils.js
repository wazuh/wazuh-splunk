...
export default function (props) {
  ...
  function removeAllChildrenValue(value, item) {
    ...
    item[childrenKey].forEach(function (n) {
      removedValue = removedValue.concat(_.remove(value, function (v) {
        return v === n[valueKey];
      }));
  ...
  function splitValue(item, checked, value, uncheckableItemValues) {
    ...
    if (checked) {
      ...
      removedValue = _.remove(nextValue, function (v) {
        // 删除自己
        if (v === itemValue) {
          return true;
        }
    ...
    var uniqValue = _.uniq(nextValue);

    var uniqRemovedValue = _.uniq(removedValue);
  ...
  function transformValue(value, flattenData, uncheckableItemValues) {
    ...
    var _loop2 = function _loop2(i) {
      ...
      tempRemovedValue = _.uniq(tempRemovedValue.concat(sv.removedValue)); // 获取到所有相关的值
