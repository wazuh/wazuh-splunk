...
function (_React$PureComponent) {
  ...
  function TimeDropdown() {
    ...
    _this.handleClick = function (type, d, event) {
      ...
      switch (type) {
        case 'hours':
          nextDate = (0, _dateFns.setHours)(date, d);
          break;

        case 'minutes':
          nextDate = (0, _dateFns.setMinutes)(date, d);
          break;

        case 'seconds':
          nextDate = (0, _dateFns.setSeconds)(date, d);
          break;
      }
  ...
  _proto.getTime = function getTime(props) {
    ...
    if (/(H|h)/.test(format)) {
      nextTime.hours = (0, _dateFns.getHours)(time);
    }

    if (/m/.test(format)) {
      nextTime.minutes = (0, _dateFns.getMinutes)(time);
    }

    if (/s/.test(format)) {
      nextTime.seconds = (0, _dateFns.getSeconds)(time);
    }
  ...
  _proto.renderColumn = function renderColumn(type, active) {
    ...
    if (!_lodash.default.isNumber(active)) {
      return null;
    }
    ...
    var hideFunc = this.props[_lodash.default.camelCase("hide_" + type)];

    var disabledFunc = this.props[_lodash.default.camelCase("disabled_" + type)];
