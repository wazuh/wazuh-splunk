...
function _default(props) {
  ...
  function removeAllChildrenValue(value, item) {
    ...
    item[childrenKey].forEach(function (n) {
      removedValue = removedValue.concat(_lodash.default.remove(value, function (v) {
        return v === n[valueKey];
      }));
  ...
  function splitValue(item, checked, value, uncheckableItemValues) {
    ...
    if (checked) {
      ...
      removedValue = _lodash.default.remove(nextValue, function (v) {
        // 删除自己
        if (v === itemValue) {
          return true;
        }
    ...
    var uniqValue = _lodash.default.uniq(nextValue);

    var uniqRemovedValue = _lodash.default.uniq(removedValue);
  ...
  function transformValue(value, flattenData, uncheckableItemValues) {
    ...
    var _loop2 = function _loop2(i) {
      ...
      tempRemovedValue = _lodash.default.uniq(tempRemovedValue.concat(sv.removedValue)); // 获取到所有相关的值

      nextValue = _lodash.default.uniq(nextValue.concat(sv.value));
    };
