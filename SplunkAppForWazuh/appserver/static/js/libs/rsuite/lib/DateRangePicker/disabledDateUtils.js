...
function isAfterDay(date1, date2) {
  return (0, _dateFns.isAfter)(new Date(date1.getFullYear(), date1.getMonth(), date1.getDate()), new Date(date2.getFullYear(), date2.getMonth(), date2.getDate()));
}

function isBeforeDay(date1, date2) {
  return (0, _dateFns.isBefore)(new Date(date1.getFullYear(), date1.getMonth(), date1.getDate()), new Date(date2.getFullYear(), date2.getMonth(), date2.getDate()));
}
/**
  ...
  return function (date, selectValue, selectedDone, target) {
    ...
    if (selectValue && selectValue[0]) {
      ...
      beforeLimit = (0, _composeFunctions.default)(function (f) {
        return (0, _dateFns.addDays)(f, -days + 1);
      }, function (f) {
  ...
  return function (date, selectValue, selectedDone, target) {
    ...
    if (selectValue && selectValue[0]) {
      ...
      beforeLimit = (0, _composeFunctions.default)(function (f) {
        ...
        return !(0, _dateFns.isSameDay)(f, date);
      })(startDate);
      afterLimit = (0, _composeFunctions.default)(function (f) {
        return (0, _dateFns.addDays)(f, days - 1);
      }, function (f) {
